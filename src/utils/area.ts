const areaList = [
    ...
    },
    {
        ...
        children: [
            ...
            { value: 820200, label: "氹仔岛", pid: 82e4, children: [{ value: 820201, label: "嘉模堂区", pid: 820200 }] },
            { value: 820300, label: "路环岛", pid: 82e4, children: [{ value: 820301, label: "圣方济各堂区", pid: 820300 }] },
        ],
    },
];
const formatterArea = (indexs: any[]) => {
    const [provinceIndex, cityIndex, countyIndex] = indexs;
    const provinceData = areaList.map((e) => {
        return e.label;
    });
    //市的数组
    const cityData = areaList[provinceIndex].children.map((e) => {
        return e.label;
    });

    //区的数组
    const areaData = areaList[provinceIndex].children[countyIndex].children.map((e) => {
        return e.label;
    });
    const arr = [provinceData, cityData, areaData]; //组装
    return arr;
};

export { areaList, formatterArea };
